...
def test_ColorSurface_init() raises:
    var surface = ColorSurface(fill=RGBA(255,0,0,255), width=100, height=100)
    assert_equal(surface.line_size, 112)
    assert_equal(surface.size, 112 * 100)
    assert_equal(surface.buffer[0], [255, 0, 0, 255])
    assert_equal(surface.buffer[112 * 100 - 1], [255, 0, 0, 255])
