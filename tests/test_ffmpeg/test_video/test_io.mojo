...
def test_video_read() raises:
    ...
    var video = video_read(root_path)
    # assert_equal(video.width, 320)
    # assert_equal(video.height, 180)
    # assert_equal(video.format, AVPixelFormat.AV_PIX_FMT_YUV420P._value)
    # assert_equal(video.n_color_spaces, 2)
...
#     var test_data_root = getenv("PIXI_PROJECT_ROOT")
#     var root_path = join(
#         test_data_root, "test_data/generate_test_videos_testsrc_320x180_30fps_2s.mp4"
#     )
#     var save_path = join(test_data_root, "test_data/test_video_save.mp4")
#     var video = video_read(root_path)
#     video_save(video, save_path)
