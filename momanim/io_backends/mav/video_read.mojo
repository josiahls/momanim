...
def decode_packet(
    ...
    while ret >= 0:
        ...
        if frame[].format != AVPixelFormat.AV_PIX_FMT_RGBA._value:
            ...
            # use the frame after this. The problem is that we are using FFmpeg
            # to allocate the frame and its buffer, so we need to use FFmpeg
            # to free the frame. The alternative, I think, is to allocate the
            # tmp frame via mojo. I'm not sure what complexity there will be in
            # this however since FFmpeg knows the internals better than me.
            video.steal_frame(
